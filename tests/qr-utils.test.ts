import { isValidUrl, isValidEmail } from '../lib/qr-utils';
...
describe('qr-utils', () => {
  ...
  describe('convertToQRCodeStylingOptions', () => {
    ...
    it('should apply custom error correction level to styling options', () => {
      const input = {
        ...getDefaultQROptions(),
        errorCorrectionLevel: ERROR_CORRECTION_LEVELS.H.value,
      };
      const output = convertToQRCodeStylingOptions(input, 'test-data');
      expect(output.qrOptions.errorCorrectionLevel).toBe(ERROR_CORRECTION_LEVELS.H.value);
    });
    ...
    it('should handle custom options correctly', () => {
      ...
      expect(stylingOptions.qrOptions.errorCorrectionLevel).toBe(ERROR_CORRECTION_LEVELS.H.value);
    });
  });
  ...
  describe('comprehensive switch statement coverage', () => {
    describe('generateQRString switch branches', () => {
      it('should handle all QR data types', () => {
        // URL type
        expect(generateQRString({ type: 'url', url: 'https://example.com' })).toBe(
          'https://example.com'
        );
  ...
  describe('utility functions', () => {
    ...
    describe('convertToQRCodeStylingOptions', () => {
      it('should convert options correctly with default values', () => {
        ...
        expect(stylingOptions.qrOptions.errorCorrectionLevel).toBe(
          defaultOptions.errorCorrectionLevel
        );
      });
      ...
      it('should handle edge cases in options conversion', () => {
        ...
        const invalidECLOptions = convertToQRCodeStylingOptions(
          { ...getDefaultQROptions(), errorCorrectionLevel: ERROR_CORRECTION_LEVELS.L.value },
          'test'
        );
        // Should still pass through the invalid value
        expect(invalidECLOptions.qrOptions.errorCorrectionLevel).toBe(
          ERROR_CORRECTION_LEVELS.L.value
        );
      });
    });
  });
